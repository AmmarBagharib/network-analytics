...
out_deg_df <- deg_df[,.(cnt=.N),by=.(to_name)]
in_deg_df <- deg_df[,.(cnt=.N),by=.(from_name)]
```
...
exports_plot <- ggplot(out_deg_df, aes(x = cnt)) +
  geom_histogram(binwidth = 10, fill = "grey", color = "black") +  # Adjust binwidth and colors
  labs(
    title = "Histogram of Out Degree (Exports)",
    x = "Degree",
    y = "Number of Countries Exported to"
  ) + scale_x_continuous(breaks=seq(0,161,10))
...
# Create a histogram using ggplot2
imports_plot <- ggplot(in_deg_df, aes(x = cnt)) +
  geom_histogram(binwidth = 10, fill = "grey", color = "black") +  # Adjust binwidth and colors
  labs(
    title = "Histogram of In Degree (Imports)",
    x = "Degree",
    y = "Number of Countries Exported to"
  ) + scale_x_continuous(breaks=seq(0,161,10))
...
Based on Figure \@ref(fig:degree-plot)
The degree distribution provides insights into how countries in the trade network are connected or how many connections (edges) they have. 

### Closeness Distribution

```{r close-plot, fig.cap="Closeness Distribution"}
hist(
  closeness(g2000),
...
Based on Figure \@ref(fig:filtered-close-plot) 121 out of 161 countries have closeness centrality of between 0.000775 and 0.000975, which is relatively very high.
...
- The presence of 19 countries with missing closeness centrality values suggests that these countries might only accept imports and not do exports.
...
Based on Figure \@ref(fig:filtered-eigen-plot) approximately 60 countries have eigen centrality values of between 0 to 0.001, and this forms the majority of the countries.  
